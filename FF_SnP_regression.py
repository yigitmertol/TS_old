...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            hidden_layer = tf.layers.dense(hidden_layer, 512, activation=tf.nn.relu, name="hidden_layer")
            hidden_layer = tf.layers.dense(hidden_layer, 256, activation=tf.nn.relu, name="hidden_layer2")
            hidden_layer = tf.layers.dense(hidden_layer, 32, activation=tf.nn.relu, name="hidden_layer3")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", default='ABT-volatiliy', type=str, help="The predicted value")


    parser.add_argument("--inputs", default='target-all,normal_close,normal_high', type=str,
    ...
    # prcs.normalize()
    prcs.make_sliding_wind_samples(_slide=1)
